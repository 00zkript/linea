    ...
    <div class="table-responsive">
        <table class="table table-bordered">
            ...
            @foreach($sections AS $item)
                <tr>
                    <td>{{ str_pad($item->idsection,7,'0000000',STR_PAD_LEFT)}}</td>
                    <td>{{ $item->nombre }}</td>
                    <td>{!! $item->estado ? '<label class="badge badge-success">Habilidado</label>' : '<label class="badge badge-danger">Inhabilitado</label>' !!}</td>
                    <td class="text-center">
                        <div class="dropdown">
                            <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenu-{{$item->idsection}}" data-toggle="dropdown" data-boundary="viewport" aria-haspopup="true" aria-expanded="false">
                                Seleccione
                            </button>
                            <div class="dropdown-menu" aria-labelledby="dropdownMenu-{{$item->idsection}}" data-idsection="{{$item->idsection}}">
        ...
        <p>
            <input type="hidden" name="paginaActual" id="paginaActual" value="{{$sections->currentPage()}}">
            Mostrando del registro {{$sections->firstItem()}} al {{$sections->lastItem()}} de un total de {{$sections->total()}} registros
        </p>
