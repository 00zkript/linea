    ...
    <div class="table-responsive">
        <table class="table table-bordered">
            ...
            @foreach($registros AS $item)
                <tr>
                    <td>{{ str_pad($item->idinstagram,7,'0000000',STR_PAD_LEFT)}}</td>
                    <td class="text-center text-nowrap">
                        <img class="img-thumbnail" style="width: 120px;height: 80px" src="{{ asset("panel/img/instagram/".$item->imagen) }}" alt="">
                    </td>
                    <td>{{ $item->nombre}}</td>
                    <td>{!! $item->estado ? '<label class="badge badge-success">Habilidado</label>' : '<label class="badge badge-danger">Inhabilitado</label>' !!}</td>
                    <td class="text-center">
                        <div class="dropdown">
                            <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenu-{{$item->idinstagram}}" data-toggle="dropdown" data-boundary="viewport" aria-haspopup="true" aria-expanded="false">
                                Seleccione
                            </button>
                            <div class="dropdown-menu" aria-labelledby="dropdownMenu-{{$item->idinstagram}}" data-idinstagram="{{$item->idinstagram}}">
